...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('Existing wallet check:', { userId, existingWallet: existingWallet ? 'found' : 'not found' });
    ...
    console.log('CDP wallet created:', { address: cdpWallet.address, walletId: cdpWallet.walletId });
    ...
    console.log('User updated with wallet ID:', { userId, walletId: wallet.id });
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Get user's wallet
    const userWallet = await getUserWallet(session.userId as string);
    if (!userWallet) {
...
async function storeWalletMapping(walletId: string, cdpWalletId: string): Promise<void> {
  try {
    const fs = await import('fs/promises');
    const path = await import('path');
    
    const mappingPath = path.join(process.cwd(), 'src/data/wallet-mapping.json');
