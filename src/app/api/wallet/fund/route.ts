...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!cdpWalletId) {
      return NextResponse.json({ error: 'CDP Wallet ID is required' }, { status: 400 });
    }
    ...
    if (error instanceof Error) {
      ...
      if (error.message.includes('already funded')) {
        return NextResponse.json(
          { error: 'Wallet has already been funded recently. Please wait before requesting more funds.' },
          { status: 400 }
        );
      }
    }
