...
import { env } from '~/env';
import type { CustodialWallet, Consumer, Transaction } from '~/types';
...
export async function getWalletByConsumerId(consumerId: string): Promise<CustodialWallet | null> {
  try {
...
export async function creditConsumerWallet(
  ...
  try {
    ...
    const balanceUpdated = await updateWalletBalance(wallet.id, currency, amount, 'add');
    if (!balanceUpdated) {
      return { success: false };
    }
...
export async function processWithdrawal(
  ...
  try {
    ...
    // Update wallet balance
    const balanceUpdated = await updateWalletBalance(wallet.id, currency, amount, 'subtract');
    if (!balanceUpdated) {
...
async function recordTransaction(transaction: Transaction): Promise<void> {
  try {
    ...
    const transactionsPath = path.join(process.cwd(), 'src/data/transactions.json');
...
async function updateTransactionStatus(transactionId: string, status: 'pending' | 'completed' | 'failed'): Promise<void> {
  try {
    const fs = await import('fs/promises');
    const path = await import('path');
...
export async function getTransactionHistory(consumerId: string): Promise<Transaction[]> {
  try {
    const fs = await import('fs/promises');
    const path = await import('path');
...
export function validateWithdrawalLimits(amount: string, currency: 'eth' | 'usdc'): { valid: boolean; error?: string } {
  ...
  if (numAmount < limit.min) {
    return { valid: false, error: `Minimum withdrawal is ${limit.min} ${currency.toUpperCase()}` };
  }
  
  if (numAmount > limit.max) {
    return { valid: false, error: `Maximum withdrawal is ${limit.max} ${currency.toUpperCase()}` };
  }
